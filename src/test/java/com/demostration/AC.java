package com.demostration;

import com.actions.OpenBrowserAction;
import com.actions.matrix.LoginAction;
import com.actions.matrix.orgCust.AssignCustAction;
import com.alibaba.fastjson.JSONObject;
import com.driver.$;
import com.pageObject.matrix.orgCust.AssignCust;
import com.utils.jdbc.JDBC;
import com.utils.list.ListMisc;
import com.utils.log.LoggerController;
import com.utils.num.IntMisc;
import com.utils.random.Randoms;
import org.openqa.selenium.WebElement;
import org.testng.annotations.AfterTest;
import org.testng.annotations.BeforeTest;
import org.testng.annotations.Test;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;

public class AC extends $ {
//    private AssignCustAction assignCustAction = new AssignCustAction();
//    private AssignCust assignCust = new AssignCust();
//
//    private final  LoggerController log = LoggerController.getLogger(AC.class);
//    private  String sql;
//    private  String originalSql1 = "";
//    private  String originalSql2 = "";
//    private  String sql1 = "";
//    private  String sql2 = "";
//    private  String sql3 = "";
//    private  String sql4 = "";
//    private  String pagination[] =  new String[]{"",""};
//    private  String where = "";
//    private  String whereLike = "";
//    private  String order = "";
//    private  String attributeValue;//通过getAttribute()获得
//    private  String string; //通用的string,用来存string
//    private  int randomPerPageNum; //随机的每页....行
//    private  int randomPageNum;
//    private  int maxPageNum;
//    private  int perPageNum;
//    private  String text;
//    private  List<List<String>> tableContent = new ArrayList<>();
//    private  JSONObject jsonObject;
//
//    @Test
//    public  void test01() {
//
//        assignCustAction.getTable();//获取第一页页面信息
//        perPageNum=assignCustAction.getPerPageNum();//获得当前页面的条/页
//        pagination = JDBC.getPagination(1, perPageNum);//获得分页语句
//        compareWebPageWithDatabase();//比较页面信息和数据库信息
//
//    }
//
//    @Test
//    public  void test02() {
//
//        maxPageNum = assignCustAction.getLastPageTable();//跳转最后一页,并获得页面信息
//        perPageNum=assignCustAction.getPerPageNum();//获得当前页面的条/页
//        pagination = JDBC.getPagination(maxPageNum, perPageNum);//获得分页语句
//        compareWebPageWithDatabase();//比较页面信息和数据库信息
//    }
//
//
//    @Test
//    public  void test03() {
//        randomPageNum =assignCustAction.getRandomPageTable();//随机生成N1,随机跳转第N1页,获得页面信息
//        perPageNum=assignCustAction.getPerPageNum();//获得当前页面的条/页
//        pagination = JDBC.getPagination(randomPageNum, perPageNum);//获得分页语句
//        compareWebPageWithDatabase();//比较页面信息和数据库信息
//    }
//
//
//    @Test
//    public  void test04() {
//        randomPerPageNum =assignCustAction.getRandomPerPageNum();//获取随机的条/页,N2
//        assignCustAction.changeToPerPage(randomPerPageNum);//更改为N2条/页
//        randomPageNum = assignCustAction.getRandomPageTable();//随机生成N1,随机跳转第N1页,获得页面信息
//        pagination = JDBC.getPagination(randomPageNum, randomPerPageNum);//获得分页语句
//        compareWebPageWithDatabase();//比较页面信息和数据库信息
//    }
//
//    @Test
//    public  void test05() {
//        get(assignCust.assignCust);
//        string = assignCustAction.getSqlResultStr(sql1);//查询数据库获得数据库中存在的cust_name
//        assignCustAction.searchCustName(string);//用客户名称查询条件搜索
//
//        where = JDBC.getWhereLike("cust_name", string);//获取where
//
//        perPageNum=assignCustAction.getPerPageNum();//获得当前页面的条/页
//        pagination = JDBC.getPagination(1, perPageNum);//获得分页语句
//
//        compareWebPageWithDatabase();//比较页面信息和数据库信息
//    }
//
//    @Test
//    public  void test06() {
//
//       string = assignCustAction.getPartialSqlResultStr(sql1);//查询数据库获得数据库中存在的cust_name(部分)
//        assignCustAction.searchCustName(string);//用客户名称查询条件搜索
//        whereLike = JDBC.getWhereLike("cust_name", string);//获取where
//        perPageNum=assignCustAction.getPerPageNum();//获得当前页面的条/页
//        pagination = JDBC.getPagination(1, perPageNum);//获得分页语句
//        compareWebPageWithDatabase();//比较页面信息和数据库信息
//    }
//
//    @Test
//    public  void test07() {
//
//        string = "fsdf";
//        assignCustAction.searchCustName(string);//用客户名称查询条件搜索
//        whereLike = JDBC.getWhereLike("cust_name", string);//获取where
//        perPageNum=assignCustAction.getPerPageNum();//获得当前页面的条/页
//        pagination = JDBC.getPagination(1, perPageNum);//获得分页语句
//        compareWebPageWithDatabase();//比较页面信息和数据库信息
//    }
//
//
//    @Test
//    public  void test08() {
//        //get(assignCust.assignCust);
//        string = assignCustAction.getSqlResultStr(sql2);//查询数据库获得数据库中存在的cust_oa
//        assignCustAction.searchOAAccount(string);//用客户oa查询条件搜索
//
//        where = JDBC.getWhereLike("cust_oa", string);//获取where
//
//        perPageNum=assignCustAction.getPerPageNum();//获得当前页面的条/页
//        pagination = JDBC.getPagination(1, perPageNum);//获得分页语句
//
//        compareWebPageWithDatabase();//比较页面信息和数据库信息
//    }
//
//    @Test
//    public  void test09() {
//       string = assignCustAction.getPartialSqlResultStr(sql2);//查询数据库获得数据库中存在的cust_name(部分)
//        assignCustAction.searchOAAccount(string);//用客户名称查询条件搜索
//        whereLike = JDBC.getWhereLike("cust_oa", string);//获取where
//        perPageNum=assignCustAction.getPerPageNum();//获得当前页面的条/页
//        pagination = JDBC.getPagination(1, perPageNum);//获得分页语句
//        compareWebPageWithDatabase();//比较页面信息和数据库信息
//    }
//
//    @Test
//    public  void test10() {
//        string = "-----";
//        assignCustAction.searchOAAccount(string);//用客户名称查询条件搜索
//        whereLike = JDBC.getWhereLike("cust_oa", string);//获取where
//        perPageNum=assignCustAction.getPerPageNum();//获得当前页面的条/页
//        pagination = JDBC.getPagination(1, perPageNum);//获得分页语句
//        compareWebPageWithDatabase();//比较页面信息和数据库信息
//    }
//
//    @Test
//    public  void test11() {
//        boolean isDisplayed = !isNotDisplayed(assignCust.getJson(assignCust.d27));
//        System.out.println(isDisplayed + "未点击展开前");
//
//        WebElement element =findElement(assignCust.getJson(assignCust.d5));
//        click(element);//点击展开
//
//        isDisplayed = !isNotDisplayed(assignCust.getJson(assignCust.d27), 4);
//        System.out.println(isDisplayed + "点了收起后");
//
//        element =findElement(assignCust.getJson(assignCust.d5));
//        click(element);//点击收起
//
//        isDisplayed = !isNotDisplayed(assignCust.getJson(assignCust.d27));
//        System.out.println(isDisplayed + "再次点了展开后");
//
//    }
//
//    @Test
//    public  void test12() {
//        //get(assignCust.assignCust);
//        string = assignCustAction.getSqlResultStr(sql3);//查询数据库获得数据库中存在的username(客户经理)
//        assignCustAction.searchCustManager(string);//用客户经理查询条件搜索
//
//        where = JDBC.getWhereLike("username", string);//获取where
//
//        perPageNum=assignCustAction.getPerPageNum();//获得当前页面的条/页
//        pagination = JDBC.getPagination(1, perPageNum);//获得分页语句
//
//        compareWebPageWithDatabase();//比较页面信息和数据库信息
//    }
//
//    @Test
//    public  void test13() {
//
//       string = assignCustAction.getPartialSqlResultStr(sql3);//查询数据库获得数据库中存在的username(客户经理)(部分)
//        assignCustAction.searchCustManager(string);//用客户经理查询条件搜索
//        whereLike = JDBC.getWhereLike("username", string);//获取where
//        perPageNum=assignCustAction.getPerPageNum();//获得当前页面的条/页
//        pagination = JDBC.getPagination(1, perPageNum);//获得分页语句
//        compareWebPageWithDatabase();//比较页面信息和数据库信息
//    }
//
//    @Test
//    public  void test14() {
//
//        string = "-----";
//        assignCustAction.searchCustManager(string);//用username(客户经理)查询条件搜索
//        whereLike = JDBC.getWhereLike("cust_oa", string);//获取where
//        perPageNum=assignCustAction.getPerPageNum();//获得当前页面的条/页
//        pagination = JDBC.getPagination(1, perPageNum);//获得分页语句
//        compareWebPageWithDatabase();//比较页面信息和数据库信息
//    }
//
//
//    @Test
//    public  void test15() {
//        get(assignCust.assignCust);//刷新页面
//        WebElement element =findElement(assignCust.getJson(assignCust.dd1));//找到客户分配按钮
//        click(element);//点击
//        boolean isDisplayed = true;
//        try {
//            findElement(assignCust.getJson(assignCust.dd3));
//        }//应该会提示 请选择客户信息
//        catch (Exception e) {
//            isDisplayed = false;
//        }
//        System.out.println(isDisplayed);
//
//    }
//
//    @Test
//    public  void test16() {
//        get(assignCust.assignCust);//刷新页面
//        WebElement element =findElement(assignCust.getJson(assignCust.dd2));//找到客户分配按钮
//        click(element);//点击
//        boolean isDisplayed = true;
//        try {
//            findElement(assignCust.getJson(assignCust.dd4));
//        }//应该会提示 请选择客户信息
//        catch (Exception e) {
//            isDisplayed = false;
//        }
//        System.out.println(isDisplayed);
//
//    }
//
//    @Test
//    public  void test17() {
//        boolean flag = true;
//        WebElement element =findElement(assignCust.getJson(assignCust.d31));
//        click(element);
//        List<WebElement> elements =findElements(assignCust.getJson(assignCust.d10));
//        int size1 = elements.size();//获取有多少行
//        try {
//            findElements(assignCust.getJson(assignCust.d10Selected));
//            int size2 = elements.size();//获取打钩的有多少行
//            if (!(size1 == size2)) {
//                flag = false;
//            }
//
//        }//应该会提示 请选择客户信息
//        catch (Exception e) {
//            flag = false;
//        }
//        System.out.println(flag);
//        flag = true;
//
//        element =findElement(assignCust.getJson(assignCust.d31));
//        click(element);
//
//        findElements(assignCust.getJson(assignCust.d10));
//        int size3 = elements.size();//获取有多少行
//        try {
//            findElements(assignCust.getJson(assignCust.d10NotSelected));
//            int size4 = elements.size();//获取打钩的有多少行
//            if (!(size3 == size4)) {
//                flag = false;
//            }
//
//        }//应该会提示 请选择客户信息
//        catch (Exception e) {
//            flag = false;
//        }
//        System.out.println(flag);
//
//    }
//
//    @Test
//    public  void test18() {
//        boolean flag = true;
//        List<WebElement> elements =findElements(assignCust.getJson(assignCust.d10));
//        int size1 = elements.size();//获取有多少行
//        int random = Randoms.getRandomNum(1,size1);
//        WebElement element = elements.get(random);
//        click(element);
//        findElements(assignCust.getJson(assignCust.d10));
//        element = elements.get(random);
//        attributeValue= getAttribute(element,"class");
//        if(!attributeValue.contains("selected")){
//            flag = false;
//        }
//        System.out.println(flag);
//    }
//
//
//    @Test
//    public  void test19() {
//        string = getRandomPageRandomLine();
//        where = JDBC.getWhere("history_cust_name",string);
//
//        assignCustAction.getPoppedTable();//获取第一页页面信息
//        perPageNum= assignCustAction.getPoppedPerPageNum();//获得当前页面的条/页
//        pagination = JDBC.getPagination(1, perPageNum);//获得分页语句
//        comparePoppedWebPageWithDatabase();//比较页面信息和数据库信息
//
//    }
//
//
//    @Test
//    public  void test20() {
//        get(assignCust.assignCust);
//        string = getRandomPageRandomLine();
//        //click();
//        WebElement element =findElement(assignCust.getJson(assignCust.dd1));
//        click(element);
//        element = findElement(assignCust.getJson(assignCust.dd1Pdd1));
//        String custName = getTextareaValue(element);
//        boolean flag = true;
//        flag = custName.equals(string);
//        System.out.println(flag);
//
//    }
//    @Test
//    public  void test21() {
//        test20();
//        WebElement element =findElement(assignCust.getJson(assignCust.dd1Pdd2));
//        click(element);
//
//        boolean flag = true;
//        try {
//            findElements(assignCust.getJson(assignCust.dd1Pdd2l1));
//            findElements(assignCust.getJson(assignCust.dd1Pdd2l2));
//        }catch (Exception e){
//            flag = false;
//        }
//
//        System.out.println(flag);
//
//    }
//
//    @Test
//    public  void test22() {
//        test21();
//        boolean flag = true;
//        WebElement element =findElement(assignCust.getJson(assignCust.dd1Pdd2l1));
//        click(element);
//        try {
//            findElements(assignCust.getJson(assignCust.dd1Pdd5));
//        }catch (Exception e){
//            flag = false;
//        }
//        System.out.println(flag);
//    }
//
//    @Test
//    public  void test23() {
//        test22();
//        WebElement element=findElement(assignCust.getJson(assignCust.dd1Pdd2));
//        click(element);
//        boolean flag = true;
//        element =findElement(assignCust.getJson(assignCust.dd1Pdd2l2));
//        click(element);
//        try {
//            findElements(assignCust.getJson(assignCust.dd1Pdd6));
//        }catch (Exception e){
//            flag = false;
//        }
//        System.out.println(flag);
//
//    }
//
//    @Test
//    public  void test24() {
//        test23();
//        findElement(assignCust.getJson(assignCust.dd1Pdd2i2));
//        WebElement element =findElement(assignCust.getJson(assignCust.dd1Pdd2i1));
//        click(element);
//
//        boolean flag1 = true;
//        boolean flag2 = true;
//
//        jsonObject = assignCust.getJson(assignCust.dd1Pdd6); //团队
//
//
//        flag1 = isNotDisplayed(jsonObject);
//
//        jsonObject = assignCust.getJson(assignCust.dd1Pdd5); //客户经理
//
//        flag2 = isNotDisplayed(jsonObject);
//
//        System.out.println(flag1&&flag2);
//    }
//
//
//    @Test
//    public  void test25() {
//        test24();
//
//
////        findElement(assignCust.getJson(assignCust.dd1Pdd3Pdd1));
////        assignCustAction.getSqlResultStr(sql4);
////        sendKeys(sql4);
////        findElement(assignCust.getJson(assignCust.dd1Pdd3Pdd1dd1));
////        click();
//        List<String> list1 = new ArrayList<>();
//
//        inputRandomDept();
//        list1=ListMisc.listAdd(list1,string);
//
//        inputRandomDept();
//        list1=ListMisc.listAdd(list1,string);
//
//        inputRandomDept();
//        list1=ListMisc.listAdd(list1,string);
//
//
//
//        WebElement element= findElement(assignCust.getJson(assignCust.dd1Pdd3));
//        String inputValue = getInputValue(element);
//
//        String[] a =inputValue.split(",");
//
//        List<String> list2 = Arrays.asList(a);
//
//        boolean flag = ListMisc.isEqualAfterSorted(list1,list2);
//
//        System.out.println(flag);
//
//
//    }
//
//
//    @Test
//    public  void test26() {
//        test25();
//        WebElement element =findElement(assignCust.getJson(assignCust.dd1Pdd2));
//        click(element);
//        element = findElement(assignCust.getJson(assignCust.dd1Pdd2l1));
//        click(element);
//        findElement(assignCust.getJson(assignCust.dd1Pdd5));
//        element =findElement(assignCust.getJson(assignCust.dd1Pdd5s1));
//        click(element);
//        element = findElement(assignCust.getJson(assignCust.dd1Pdd5Pdd1));
//        sendKeys(element,"金钊");//这里写死了,以后可以改一下
//        element=findElement(assignCust.getJson(assignCust.dd1Pdd5Pdd2));
//        click(element);
//        element =findElement(assignCust.get$Json1P("last()"));
//        click(element);
//        element =findElement(assignCust.getJson(assignCust.dd1Pdd5Pdd4));
//        click(element);
//
//
//       element=findElement(assignCust.getJson(assignCust.dd1Pdd5));
//        //金钊
//        String a = getInputValue(element);
//        boolean flag = true;
//        flag=a.equals("金钊");
//        System.out.println(flag);
//
//    }
//
//
//    @Test
//    public  void test27() {
//        test26();
//        WebElement element = findElement(assignCust.getJson(assignCust.dd1Pdd4));
//        String random = Randoms.getRandomStr(Randoms.getRandomNum(5,200));
//        sendKeys(element,random);
//
//
//        findElement(assignCust.getJson(assignCust.dd1Pdd1));
//        String custName = getTextareaValue(element);
//
//        where = JDBC.getWhere("history_cust_name",custName);
//        order = "order by sy_createtime desc";
//        List<List<String>> list1 = getResultToList();
//
//        element =findElement(assignCust.getJson(assignCust.d30));
//        click(element);
//
//
//        if (isNotDisplayed(assignCust.getJson(assignCust.d17))) {//如果数据加载的那个菊花不显示了,说明已经加载好
//            where = JDBC.getWhere("history_cust_name",custName);
//        }
//
//
//        order = "order by sy_createtime desc";
//        List<List<String>> list2 = getResultToList();
//
//        boolean flag = true;
//        flag = ListMisc.isEqual(list1,list2);
//        System.out.println(flag);
//
//
//    }
//
//    @Test
//    public  void test28() {
//        test26();
//
//        WebElement element = findElement(assignCust.getJson(assignCust.dd1Pdd4));
//        String random = Randoms.getRandomStr(Randoms.getRandomNum(5,200));
//        sendKeys(element,random);
//
//
//        element=findElement(assignCust.getJson(assignCust.dd1Pdd1));
//        String custName = getTextareaValue(element);
//
//        where = JDBC.getWhere("history_cust_name",custName);
//        order = "order by sy_createtime desc";
//        List<List<String>> list1 = getResultToList();
//
//        element =findElement(assignCust.getJson(assignCust.d29));
//        click(element);
//
//        where = JDBC.getWhere("history_cust_name",custName);
//        order = "order by sy_createtime desc";
//        List<List<String>> list2 = getResultToList();
//
//        boolean flag = true;
//        flag = !ListMisc.isEqual(list1,list2,false);
//        System.out.println(flag);
//
//
//    }
//
//    @Test
//    public  void test29() {
//        test21();
//        WebElement element =findElement(assignCust.getJson(assignCust.dd1Pdd2l2));
//        click(element);
//        element =findElement(assignCust.getJson(assignCust.dd1Pdd6i1));
//        click(element);
//        element = findElement(assignCust.getJson(assignCust.dd1Pdd6Pdd1));
//        sendKeys(element,"上海陆家嘴东路营业部");//这里写死了,可能可以优化
//        element =findElement(assignCust.get$JsonTeam("上海陆家嘴东路营业部"));
//        click(element);
//        element =findElement(assignCust.getJson(assignCust.dd1Pdd6Pdd2));
//        click(element);
//        element =findElement(assignCust.getJson(assignCust.d29));
//        click(element);
//
//
//
//    }
//
//
//    @Test
//    public  void test30() {
//        test20();
//        WebElement element =findElement(assignCust.getJson(assignCust.d29));
//        click(element);
//    }
//
//
//
//        @BeforeTest
//    public  void beforeTest01() {
//        OpenBrowserAction.open("chrome");
//        LoginAction.login();
//        get(assignCust.assignCust);//登录后跳转客户维护分配页面
//
////        originalSql = "select *\n" +
////                "  from (select rownum as 编号,\n" +
////                "               cust_name as 客户名称,\n" +
////                "               replace_code_with_name(cust_grade, 'MR_CM_GRADE') as 客户级别,\n" +
////                "               cust_oa as OA账号,\n" +
////                "               deptname as 客户所属部门,\n" +
////                "               username as 客户经理,\n" +
////                "               user_relation_team as 团队名称\n" +
////                "          from CUST_WH_RELATION where 1=1 ) where 1 = 1\n";
//
//
//        originalSql1 = "   select * from (select rownum as row_id, tmp.* from (select  \n" +
//                "               cust_name as 客户名称,\n" +
//                "               replace_code_with_name(cust_grade, 'MR_CM_GRADE') as 客户级别,\n" +
//                "               cust_oa as OA账号,\n" +
//                "               deptname as 客户所属部门,\n" +
//                "               username as 客户经理,\n" +
//                "               user_relation_team as 团队名称\n" +
//                "          from CUST_WH_RELATION where 1=1  ) tmp  where 2=2 ) where 3=3 ";
//
//
//        originalSql2 ="\n" +
//                " select * from (select rownum as row_id, tmp.* from (select  \n" +
//                "               history_cust_name as 客户名称,\n" +
//                "               history_manager as 客户维护经理,\n" +
//                "               history_oa as OA账号,\n" +
//                "               history_dept_name as 部门,\n" +
//                "               history_reson as 原因\n" +
//                "          from CUST_WH_HISTORY where 1=1 ) tmp  where 2=2 ) where 3=3 ";
//
//
//        sql1 = "select 客户名称\n" +
//                "  from (select cust_name as 客户名称\n" +
//                "          from CUST_WH_RELATION\n" +
//                "         where cust_name is not null\n" +
//                "         order by dbms_random.value)\n" +
//                " where rownum = 1";
//
//        sql2 = "select OA账号\n" +
//                "  from (select cust_oa as OA账号\n" +
//                "          from CUST_WH_RELATION\n" +
//                "         where cust_oa is not null\n" +
//                "         order by dbms_random.value)\n" +
//                " where rownum = 1";
//
//
//        sql3 = "select 客户经理\n" +
//                "  from (select username as 客户经理\n" +
//                "          from CUST_WH_RELATION\n" +
//                "         where username is not null\n" +
//                "         order by dbms_random.value)\n" +
//                " where rownum = 1";
//
//        sql4 = "select 部门\n" +
//                "  from (select DEPTNAME as 部门\n" +
//                "          from JE_CORE_DEPARTMENT\n" +
//                "         where deptname is not null and status = '1'\n" +
//                "         order by dbms_random.value)\n" +
//                " where rownum = 1\n";
//
//    }
//
//
//    private  void compareWebPageWithDatabase() {
//        List<List<String>> list1 = tableContent;//把页面内容放入list里
//
//        sql = originalSql1;//拼上分页语句
//
//        String pagination1 = pagination[0];
//        String pagination2 = pagination[1];
//
//        sql = sql.replaceFirst("where 2=2", "where 2=2 " + pagination1);
//        sql = sql.replaceFirst("where 3=3", "where 3=3 " + pagination2);
//
//        if (!where.equals("")) {
//            sql = sql.replaceFirst("where 1=1", "where 1=1 " + where + order);
//        } else {
//            sql = sql.replaceFirst("where 1=1", "where 1=1 " + whereLike + order);
//        }
//
//
//        List<List<String>> list2 = new ArrayList<>();
//
//        list2 = JDBC.getResultToList(sql);//获得sql结果,放入list2
//
//        boolean flag = ListMisc.isEqual(list1, list2);//判断网页上的内容和数据库内容是否相符
//
//        sql = "";
//        where = "";
//        whereLike = "";
//        order = "";
//        pagination = new String[]{"",""};
//
//        WebElement element =findElement(assignCust.getJson(assignCust.d4));
//        click(element);
//        System.out.println(flag);
//
//
//    }
//
//
//    private  List<List<String>>  getResultToList(){
//        sql = originalSql2;//拼上分页语句
//
//        String pagination1 = pagination[0];
//        String pagination2 = pagination[1];
//
//        sql = sql.replaceFirst("where 2=2", "where 2=2 " + pagination1);
//        sql = sql.replaceFirst("where 3=3", "where 3=3 " + pagination2);
//
//        if (!where.equals("")) {
//            sql = sql.replaceFirst("where 1=1", "where 1=1 " + where);
//        } else {
//            sql = sql.replaceFirst("where 1=1", "where 1=1 " + whereLike);
//        }
//
//
//        List<List<String>> list2 = new ArrayList<>();
//
//        list2 = JDBC.getResultToList(sql);//获得sql结果,放入list2
//
//        sql = "";
//        where = "";
//        whereLike = "";
//        pagination = new String[]{"",""};
//
//        return  list2;
//    }
//
//
//
//
//    private  void comparePoppedWebPageWithDatabase() {
//        List<List<String>> list1 = tableContent;//把页面内容放入list里
//
//        sql = originalSql2;//拼上分页语句
//
//        String pagination1 = pagination[0];
//        String pagination2 = pagination[1];
//
//        sql = sql.replaceFirst("where 2=2", "where 2=2 " + pagination1);
//        sql = sql.replaceFirst("where 3=3", "where 3=3 " + pagination2);
//
//        if (!where.equals("")) {
//            sql = sql.replaceFirst("where 1=1", "where 1=1 " + where);
//        } else {
//            sql = sql.replaceFirst("where 1=1", "where 1=1 " + whereLike);
//        }
//
//
//        List<List<String>> list2 = new ArrayList<>();
//
//        list2 = JDBC.getResultToList(sql);//获得sql结果,放入list2
//
//        boolean flag = ListMisc.isEqual(list1, list2);//判断网页上的内容和数据库内容是否相符
//
//        sql = "";
//        where = "";
//        whereLike = "";
//        pagination = new String[]{"",""};
//
////        findElement(assignCust.getJson(assignCust.d4));
////        click();
//        System.out.println(flag);
//
//
//    }
//
//
//    public  String getRandomPageRandomLine(){
//        findElement(assignCust.getJson(assignCust.d10));//可能电脑太快,d14里可能1的时候就读出来,所以先查表格
//        WebElement element = findElement(assignCust.getJson(assignCust.d14));
//        text = getText(element);//获取第几页,保存到text里
//        randomPageNum = IntMisc.getRandomPageNum(text);
//         element = findElement(assignCust.getJson(assignCust.d15));
//        //clear();
//        sendKeys(element,"" + randomPageNum);
//        element = findElement(assignCust.getJson(assignCust.d28));
//        click(element);//点一下页面其他元素,会开始查询
//        //已上这些是用来跳转随即页
//        List<WebElement> elements =findElements(assignCust.getJson(assignCust.d10));
//        int size1 = elements.size();//获取有多少行
//        int random = Randoms.getRandomNum(1,size1);
//        element = elements.get(random);
//        assignCustAction.getInfo(""+random);//获取第N行的信息,包括表头
////        tableContent
//        int a = ListMisc.getPosition(tableContent.get(0),"客户名称");
//        String custName = tableContent.get(1).get(a);
//        string = custName;
//        System.out.println(custName);
//        doubleClick(element);
//        return custName;
//
//    }
//
//
//    public  void inputRandomDept(){
//        WebElement element =findElement(assignCust.getJson(assignCust.dd1Pdd3));
//        click(element);
//        element = findElement(assignCust.getJson(assignCust.dd1Pdd3Pdd1));
//        string = assignCustAction.getSqlResultStr(sql4);
//        sendKeys(element,string);
//        element =findElement(assignCust.get$JsonCompany(string));
//        click(element);
//        element =findElement(assignCust.getJson(assignCust.dd1Pdd3Pdd2));
//        click(element);
//    }
//
//
//    @AfterTest
//    public  void afterTest01() throws InterruptedException {
//
//        Thread.sleep(5000);
//        $.quit();
//    }


}
